...
func NewBlock(nounce int, prevHash [32]byte, transactions []*Transaction) *Block {
	b := new(Block)
...
func NewBlockchain(blockchainAddress string) *Blockchain {
	//this is creating a new blockchain
	b := &Block{}
	bc := new(Blockchain)
...
func (bc *Blockchain) CreateBlock(nounce int, prevHash [32]byte) *Block {
	//this is createing a new block and adding it to the existing blockchain and returns the newly created block
	b := NewBlock(nounce, prevHash, bc.transactionPool)
...
func (bc *Blockchain) ValidProof(nounce int, PrevHash [32]byte, transaction []*Transaction, difficulty int) bool {
	zeros := strings.Repeat("0", difficulty)
	guessBlock := Block{0, nounce, PrevHash, transaction}
...
func (bc *Blockchain) ProofOfWork() int {
	...
	for !bc.ValidProof(nounce, previoushash, transactions, MINING_DIFFICULTY) {
		nounce += 1
	}
	return nounce
}
